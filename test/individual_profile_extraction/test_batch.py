"""
Batch file testing tool for the /extract/individualProfile endpoint.
...
Options:
    ...
    --url URL         Backend URL (default: http://localhost:8000/extract/individualProfile)
...
Examples:
    python test_batch.py --dir ./test_files
    python test_batch.py --dir ./test_files --workers 3 --format json
    python test_batch.py --dir ./test_files --url http://localhost:8000/extract/individualProfile --output ./results
"""
...
async def send_file(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            result["idType"] = data.get("idType")
            result["idNumber"] = data.get("idNumber")
            result["nationality"] = data.get("nationality")
            result["dateOfBirth"] = data.get("dateOfBirth")
            result["idIssueDate"] = data.get("idIssueDate")
            result["idExpiryDate"] = data.get("idExpiryDate")
            result["residentialAddress"] = data.get("residentialAddress")
            result["correspondenceAddress"] = data.get("correspondenceAddress")
        else:
        ...
        async def bounded_send(index: int, path: Path) -> None:
            async with semaphore:
                ...
                if result["error"]:
                    ...
                    print(
                        f"           name={result['name']}"
                        f"  idType={result['idType']}"
                        f"  idNumber={result['idNumber']}"
                        f"  nationality={result['nationality']}"
                        f"  dob={result['dateOfBirth']}"
                    )
                    print(
                        f"           issueDate={result['idIssueDate']}"
                        f"  expiryDate={result['idExpiryDate']}"
                    )
                    print(f"           residentialAddress={result['residentialAddress']}")
                    print(f"           correspondenceAddress={result['correspondenceAddress']}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch test the file-parser /extract/individualProfile endpoint with a folder of files."
    )
    parser.add_argument(
        "--dir",
        type=Path,
    ...
    parser.add_argument(
        "--url",
        default="http://localhost:8000/extract/individualProfile",
        help="Backend URL (default: http://localhost:8000/extract/individualProfile)",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    print("=" * 60)
    print(f"🚀  File Parser — Individual Profile Batch Test")
    print("=" * 60)
